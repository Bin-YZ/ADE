...
#########################   fully automated time stepping #############################
    ...
    def solve_adaptive_timesteps(self):
        ...
        A_flag =True # it is save to set it true, because dt is changing and re-assembly of A is required
        # Prepare solvers for computing tentative time steps

        solver_low = self.ADEsolver(u_n, u_np1_low,boundary_conditions,dt,theta)
        solver_high_1 = self.ADEsolver(u_n, u_np1_high,boundary_conditions,dt,theta)
        solver_high_2 = self.ADEsolver(u_np1_high, u_np1_high,boundary_conditions,dt,theta)
        ...
        t = self.__T_start
        dtreservoir=dt
        # the next works always, because there is a default array with times given
        dtreservoir = self.check_downstream_exchange_dt(t,dt) # time_steps for output...
        if dt > dtreservoir: # smaller time step should be safe..
            dt=dtreservoir        
            logging.debug('DEBUG: end of time step: dt_new changed to dtmax: '+str(dtreservoir))
        dtreservoir = self.check_upstream_exchange_dt(t,dt) # time_steps for output...    
        if dt > dtreservoir: # smaller time step should be safe..
            dt=dtreservoir
            logging.debug('DEBUG: end of time step: dt_new changed to dtmax: '+str(dtreservoir))
        ...
        while t < T :
            ...
            #if dt > dtmax:
            #    dt = dtmax
            logging.debug('DEBUG: Time step:i,t,dt '+str(icount)+" "+str(t)+" "+str(dt)+" "+str(dt))
            # Compute tentative time steps with constant boundary conditions!
            solver_low(boundary_conditions,dt,A_flag) # (t, dt)   
            solver_high_1(boundary_conditions,dt/2.0,A_flag)   # (t, dt/2)
            solver_high_2(boundary_conditions,dt/2.0,A_flag) #(t+dt, dt/2)
            ...
            if est > tol:
                ...
                dt = dt_new
                logging.debug('DEBUG during time iteration: repeat time step: t,dt '+str(t)+" "+str(dt)+" "+str(dtreservoir))
                continue
            ...
            dflux,_down_area=self.calculate_diffusive_fluxes(u_np1_high,self.__downstream_boundary_id) # this we do for downstream only
            
            uflux,_up_area=self.calculate_diffusive_fluxes(u_np1_high,self.__upstream_boundary_id) # this we do for upstream only
             
            if self.__flag_reservoirs: # only if reservoirs are used, otherwise we have constant concentration boundaries
            ...
            new_row = pd.DataFrame([t,dt,up_bc,down_bc,uflux,dflux], columns=['row1'], index=parameters).T
            
            df = pd.concat([df if not df.empty else None, new_row]) # df is empty initially, concat with empty df will be not supported in future pandas version...therefore avoid concat with empty df
                # Save to file
            ...
            # prepare initial data, increase time and get new time step size
            u_n.x.array[:] = u_np1_high.x.array[:]
            t += dt
            icount +=1
            # test for new time step size
            if self.__flag_reservoirs : # only if reservoirs are used, otherwise we have constant concentration boundaries            
                if (self.downstream_exchange(t)): # this will reset downstream concentrations for next time step, if  it fits
                    logging.debug('DEBUG downstream reservoir set to zero ')
                if (self.upstream_exchange(t)): # this will reset upstream concentrations for next time step, if  it fits
                    logging.debug('DEBUG upstream reservoir set to zero ')
            # the next works always, because there is a default array with times given                
            dtreservoir = self.check_downstream_exchange_dt(t,dt_new) # time_steps for output...
            if dt_new > dtreservoir: # smaller time step should be safe..
                dt_new=dtreservoir
                logging.debug('DEBUG: end of time step: dt_new changed to dtmax: '+str(dtreservoir))
            dtreservoir = self.check_upstream_exchange_dt(t,dt_new) # time_steps for output...    
            if dt_new > dtreservoir: # smaller time step should be safe..
                dt_new=dtreservoir
                logging.debug('DEBUG: end of time step: dt_new changed to dtmax: '+str(dtreservoir))
            dt = dt_new
            
        logging.info("INFO: Time loop finished at t="+str(t)+" after i="+str(icount)+" time steps.")
        return u_n,df
